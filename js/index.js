			...
			success: function(response) {
				...
				if (response.pass == true) {
		    	  ...
		    	  picture_path_array.push(response.picture[i]);
					...
					//记录下登陆时间
					$.ajax({
						url: newDomainname + "Update_logtime/" + cookie,
...
function generater_req_1(source_id, chapter_id) {
	...
	console.log(Object.assign({}, {
		request_index: 1,
		time_stamp: req_time_stamps[1],
		source_id: source_id,
		chapter_id: chapter_id
	}));
	console.log("send:" + Object.assign({}, {
		request_index: 1,
		time_stamp: req_time_stamps[1],
		source_id: source_id,
		chapter_id: chapter_id
...
function get_time_valid(time_index, pre_judge_time) {
	...
	return (req_time_stamps[time_index] && typeof(pre_judge_time) == 'number') ? (req_time_stamps[time_index] ===
		pre_judge_time) : false;
}
...
$(window).unload(function() {
	...
	if (!windowstatus) {
		var cookie = $.cookie("user");
		$.ajax({
			url: newDomainname + "Update_exitime/" + cookie,
			success: function(result) {
				console.log(result);
				if (result) {
					alert("您的学习状态保存成功");
